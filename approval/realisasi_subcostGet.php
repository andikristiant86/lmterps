...
$check_kasir=$dbpay->getOne("select count(*) from m_kasir where pic_id='$login_nip' and verify='Y'");
if($act=='view'){
	...
	$where_sub=($check_kasir>=1)?"where ((x.kasir_pic='$login_nip' and x.status_real in ('2'))
	or (x.pemohon='$login_nip' and x.status_real='1')) and x.status='6'":
	"where x.pemohon='$login_nip' and x.status='6' and x.status_real='1'";
	...
	CONVERT(VARCHAR(24),tanggal_disetujui,113) as bc_app_date, c.lokasi_kerja,b.lke_id,d.realisasi as amount_real,d.[back] as amount_back,d.deficit as amount_deficit,d.file_name,
	case 
							when x.status='2' then '032015' else 
	...
	$sql="select * from ($sql) as x $where order by tgl desc";
	...
	while($row=$result_user->Fetchrow()){
		...
		$lokasi_kas=($lke_id!=$lokasi)?"<b><font color=red>$lokasi_kerja</font></b>":"<b><font color=green>$lokasi_kerja</font></b>";
			...
			"phone_number"=>"$phone_number","status_real"=>"$status_real",
...
if ($_FILES["fileToUpload"]["size"] > (5*1048576)) {
	echo json_encode(array('errorMsg'=>"Sorry, max size upload 5Mb!"));
    die();
}
// Allow certain file formats
if($imageFileType != "JPG" && $imageFileType != "jpg" && $imageFileType != "JPEG" && $imageFileType != "jpeg" && 
	$imageFileType != "PNG" && $imageFileType != "png" && $imageFileType != "XLS" && $imageFileType != "xls" && 
	$imageFileType != "XLSX" && $imageFileType != "xlsx" && $imageFileType != "doc" && $imageFileType != "DOC" && $imageFileType != "pdf" && $imageFileType != "PDF"
	&& $imageFileType != "rar" && $imageFileType != "RAR") {
	echo json_encode(array('errorMsg'=>"Sorry, only jpg, pjg, pdf, xls, doc, rar files are allowed."));
    die();
}
move_uploaded_file($_FILES["fileToUpload"]["tmp_name"], $target_file);
	...
	$amount_real=empty($amount_real)?$amount_paid:$amount_real;
	
	if($check_kasir>=1){
		$dbproj->Execute("update t_pengajuan_biaya set status_real='3',realisasi='$amount_real' where [no]='$no'");
		$dbproj->Execute("update t_pengajuan_biaya_real set status='YES' where pengajuan_id='$no'");
	}else{
...
elseif($act=='do_approved'){
	...
	foreach ($rows as $key=>$val){
		...
		$realisasi=$dbproj->getOne("select cast (case when realisasi <= 0 then paid else realisasi end as int) from t_pengajuan_biaya_real where pengajuan_id='$pengajuan_id'");
		$dbproj->Execute("update t_pengajuan_biaya set status_real='3',realisasi='$realisasi' where id='$val'");
