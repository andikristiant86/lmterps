...
?>
<table id="dg" style="width:100%;height:525"
	...
	<div id="toolbar">
		<table width="100%" cellpadding=0 cellspacing=0>
			...
			<td align="right">
				<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-xls" plain="true" onclick="print()">Export</a>
			</td>
			</tr>
		</table>
	</div>
	...
	<div id="dlg" class="easyui-dialog" style="width:60%;height:445px;padding:0px 0px" closed="true" buttons="#dlg-buttons">
...
<div id="hideShow1">
	...
	<div class ="fitem">
		...
		<input name="x4" class="easyui-textbox" style="width:75%;" readOnly></input>
	</div>
	<div class ="fitem">
		<label>PROJECT NAME</label>
		<input name="x5" class="easyui-textbox" style="width:75%;" readOnly></input>
	</div>
	<div class ="fitem">
		<label>SOW NAME</label>
		<input name="x10" class="easyui-textbox" style="width:75%;" readOnly></input>
	</div>
	<div class ="fitem">
		<label>DT COORDINATOR</label>
		<input name="x6" class="easyui-textbox" style="width:75%;" readOnly></input>
	</div>
	<div class ="fitem">
		<label>DT NAME</label>
		<input name="x8" class="easyui-textbox" style="width:75%;" readOnly></input>
	</div>
	<div class ="fitem">
		<label>TOTAL</label>
		<input name="x7" class="easyui-textbox" style="width:75%;" readOnly></input>
	</div>
	
	<div class ="fitem">
		<label>STATUS</label>
		<input name="x9" class="easyui-textbox" style="width:75%;" readOnly></input>
	</div>
	
	<div class ="fitem">
		<label>DESCRIPTION</label>
		<input name="x11" class="easyui-textbox" style="width:75%;" readOnly></input>
	</div>
...
<div id="hideShow2">
	...
	data-options="
								panelWidth:400,
								url: 'RequestCarpoolGet.php?act=combo_driver',
								idField:'nip',
								textField:'nm_peg',
								mode:'remote',
								fitColumns:true,
	...
	data-options="
								panelWidth:400,
								url: 'RequestCarpoolGet.php?act=combo_car',
            ...
            $('#dg').datagrid({
				...
				onExpandRow: function(index,row){
					var ddv = $(this).datagrid('getRowDetail',index).find('table.ddv');
						...
						$('#ddv-'+index).datagrid({
                        url:'RequestCarpoolGet.php?act=detailview&ocs_id='+row.ocs_id,
					...
					function addDetail(){
						...
						// }else{
							...
							$("#hideShow1").hide();
							$("#hideShow2").show();
							$('#dlg').dialog('open').dialog('setTitle','Add Request');
							$('#fm').form('clear');
							$('#fm').form('load',{
										description:row.ocs_desc
									});
							url = 'RequestCarpoolGet.php?act=do_add&ocs_id='+row.ocs_id;
							v_index=index;
							xload=1;
						//}
					}
					function editDetail(){
						...
						if (rowx){
							...
							// }
								...
								$('#fm').form('load',rowx);
								url = 'RequestCarpoolGet.php?act=do_update&id='+rowx.id;
									$('#fm').form('load',{
						...
						if (rowx){
							if(row.sts_nm=='JALAN' || row.sts_nm=='PULANG' || row.sts_nm=='CLOSED'){
								...
								$.messager.confirm('Confirm','Are you sure you want to delete?',function(r){
									if (r){
										$.post('RequestCarpoolGet.php?act=removeDetail',{id:rowx.id},function(result){
											if (result.success){
...
function prosess_jalan(){
	...
	if (row){
		if(row.status==1){
			...
			if (r){
				$.post('RequestCarpoolGet.php?act=prosess_jalan',{ocs_id:row.ocs_id},function(result){
					if (result.success){
...
function prosess_pulang(){
	...
	if (row){
		if(row.status=='1'){
			...
			url = 'RequestCarpoolGet.php?act=prosess_pulang';
			xload=2;
...
function print (){
	var start_date 	= $('#str_date').val();
	var end_date 	= $('#upto_date').val();
	window.open("/carpool/export_carpoolExcel.php?start_date="+start_date+"&end_date="+end_date, "_blank", "toolbar=yes, scrollbars=yes, resizable=yes, top=150, left=400, width=400, height=1");
}
    </script>
