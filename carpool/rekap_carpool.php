...
<label>Type Rekap</label> <select class="easyui-combobox" name="format" style="width:20%;" id="format" data-options="panelHeight:'auto'">
	<option value="1">Carpool Real</option>
	<option value="2">Rental Real</option>
	<option value="3">Subcost Real</option>
	<option value="4">Pulsa Real</option>
	<option value="6">Rigger Real</option>
	<option value="7">Salary Real (Per-periode)</option>
	<option value="5">Budget Plan</option>
	<option value="8">Carpool Plan</option>
...
function submit(){
	...
	if(output=='html'){
		...
		if(format=='1'){
			...
			window.location.assign("rekap_salaryExcel.php?pm_id="+pm_id+"&proj_code="+proj_code+"&start_date="+start_date+"&end_date="+end_date+"&lke_id="+lke_id);
		}else{
