...
<table id="dg" style="width:100%;height:475"
			...
			data-options="
					...
					singleSelect:false,
					collapsible: true,
					url:'pooling_riggerGetNew.php?act=view',
					method:'post',
					pagination:true,
					toolbar:'#toolbar',
					striped: true,
		...
		<table width="100%" cellpadding=0 cellspacing=0>
			<tr><td>
				...
				<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-add" plain="true" 
						onclick="insert()">Insert</a>
				<!--<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-remove" plain="true" 
						onclick="deletex()">Remove</a>-->
				<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-cancel" plain="true" 
						onclick="cancelx()">Cancel</a>
				<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-ok" plain="true" 
						onclick="closedx()">Closed</a>
			</td>
			<td align="right">
			</td>
			</tr>
	...
	function detailpool(v){
		window.open("pooling_riggerDetailNew.php?v="+v, "_blank", "top=0,width=1000,height=500");
	}
...
<div id="dlg1" class="easyui-dialog" style="width:65%;height:360px;padding:10px 10px" closed="true" buttons="#dlg-buttons1">
	<form id="ff" method="post" enctype="multipart/form-data">	
		...
		<div class="fitem">
            <label>RIGGER</label>
				<select class="easyui-combogrid" name="nip" id="nip" style="width:60%" required="true" data-options="
                    ...
                    textField: 'nm_peg',
                    url: 'pooling_riggerGetNew.php?act=combo_rigger',
                    mode:'remote',
					fitColumns:true,
                    columns: [[
		...
		<div class="fitem">
            <label>AREA</label>
			<select id="area_site" class="easyui-combobox" name="area_site" labelPosition="top" style="width:40%;" panelHeight="auto">
		...
		<div class="fitem">
            <label>TRANSPORT</label>
			    <select id="transport" class="easyui-combobox" name="transport" labelPosition="top" style="width:40%;" panelHeight="auto">
        ...
        <div class="fitem">
            <label>PROJECT</label>
				<select class="easyui-combogrid" name="project" id="project" style="width:60%" required="true" data-options="
                    ...
                    textField: 'text',
                    url: 'pooling_riggerGetNew.php?act=combo_project',
                    mode:'remote',
					nowrap: false,
					fitColumns:true,
                    columns: [[
					...
					onSelect: function (index,row){
						...
						$('#site_name1').combogrid({url:'pooling_riggerGetNew.php?act=combo_site&proj_id='+row.proj_id+'&sow_id='+row.sow_id});
						$('#site_name2').combogrid({url:'pooling_riggerGetNew.php?act=combo_site&proj_id='+row.proj_id+'&sow_id='+row.sow_id});
						$('#site_name3').combogrid({url:'pooling_riggerGetNew.php?act=combo_site&proj_id='+row.proj_id+'&sow_id='+row.sow_id});
						$('#site_name4').combogrid({url:'pooling_riggerGetNew.php?act=combo_site&proj_id='+row.proj_id+'&sow_id='+row.sow_id});
						$('#site_name5').combogrid({url:'pooling_riggerGetNew.php?act=combo_site&proj_id='+row.proj_id+'&sow_id='+row.sow_id});
					}
                ">
				</select>
...
function insert(){
	...
	url = 'pooling_riggerGetNew.php?act=do_add';	
...
function deletex(){
			...
			if (row){
				$.messager.confirm('Confirm','Are you sure you want to delete this data?',function(r){
					if (r){
						$.post('pooling_riggerGetNew.php?act=do_destroy',{prg_id:row.id},function(result){
							if (result.success){
...
function cancelx(){
			var row = $('#dg').datagrid('getSelected');
			if (row){
				$.messager.prompt('CANCEL', 'Are you sure you want to cancel this data? <br><b>Please, enter a reason !!!', function(r){
                if (r){
                    $.post('pooling_riggerGetNew.php?act=do_cancel',{reason:r,prg_id:row.id},function(result){
							if (result.success){
								$('#dg').datagrid('reload'); // reload the user data
							} else {
								$.messager.alert('Error',result.errorMsg,'error');
...
function closedx(){
			var row = $('#dg').datagrid('getSelected');
			if (row){
				$.messager.confirm('Confirm','Are you sure you want to closed this data?',function(r){
					if (r){
						$.post('pooling_riggerGetNew.php?act=do_closed',{prg_id:row.id},function(result){
							if (result.success){
								$('#dg').datagrid('reload'); // reload the user data
							} else {
								$.messager.alert('Error',result.errorMsg,'error');
