...
if($act=='view'){
	...
	case isnull(a.status,'') when '1' then 'JALAN' when '5' then 'VERIFIED' when '2' then 'PULANG' when '3' then 'CLOSED' when '111' then 'REJECT' when 112 then 'CANCEL' else 'BELUM JALAN' end as sts_nm,
	...
	"where a.lke_id in (select area_id from lmt_payment.dbo.m_kasir where pic_id='$login_nip')");
	...
	$_sql="select * from 
	($sql) as x $find_str order by  ocs_id desc";
	...
	$jum_verified=$dbproj->getOne("select count(*) from ($sql) as x $find_str and sts_nm='VERIFIED' ");
	...
	$result["footer"]=array(
							...
							array("ocs_desc"=>"<b>VERIFIED</b>","sts_nm"=>"<b><span style='color:black'>$jum_verified</span></b>"),
	...
	$find_name=$_REQUEST['name'];
	$find_str=($find_name=='all' or empty($find_name))?
	"where sts_nm not in ('BELUM JALAN','CANCEL','REJECT') and (kasir_picx='$login_nip' or kasir_pic='$login_nip') and date between '$f_start_date' and '$f_end_date' and pm_approve='1' and finance_approve='1' and 
	(ocs_id like '%$find_val%' or time like '%$find_val%' or date like '%$find_val%' or site_name like '%$find_val%'
	or proj_name like '%$find_val%' or dtc_name like '%$find_val%' or sts_nm like '%$find_val%' or dt_name like '%$find_val%' or no_polisi like '%$find_val%' 
	or area like '%$find_val%'
	)":
	"where  sts_nm not in ('BELUM JALAN') a and date between '$f_start_date' and '$f_end_date' and pm_approve='1' and finance_approve='1' 
	and (kasir_picx='$login_nip' or kasir_pic='$login_nip') and $find_name like '%$find_val%'";
	...
	c.pic_sub as kasir_picx
	from m_carpool a
	left join t_carpool_payment b on b.ocs_id=a.ocs_id
	left join lmt_payment.dbo.m_kasir c on c.pic_id=a.kasir_pic AND c.TYPE='CARPOOL'
	";
	
	$result["total"] = $dbproj->getOne("select count(*) from ($sql
	) as x $find_str");
	
	$sql="select * from ($sql
	) as x $find_str order by  ocs_id desc";
	if($t=="y")die($sql);
...
elseif($act=='detailview'){
	...
	//$items[]=array("nip"=>"000000","nm_peg"=>"NO DRIVER(DI KENDARAI SENDIRI)");
	$items[]=array("nm_peg"=>"NO DRIVER(DI KENDARAI SENDIRI)","nip"=>"000000");
	$result["rows"] = $items;
	echo json_encode($result);
...
elseif($act=='prosess_jalan'){
	...
	if($cek_driver==0){
		if($nip_driver1 != '000000'){
			echo json_encode(array('errorMsg'=>'Driver wajib disi, silahkan pilih dengan benar!'));
			die();
		}
	}
	
	if(is_numeric($km_start1) == FALSE){
		echo json_encode(array('errorMsg'=>'Km start harus diisi!!!'));
		die();
	}
	...
	$dbproj->Execute("exec process_jalan_car '$ocs_id','$no_polisi','$km_start1','$nip_driver1','$kode_kendaraan1','$login_nip'");
...
elseif($act=='prosess_paid'){
	...
	$dbproj->Execute("update t_carpool set paid_date=GETDATE(), paid_by='$login_nip' where ocs_id='$ocs_id'");
	...
	$status=$dbproj->getOne("select status from m_carpool where ocs_id='$ocs_id'");
	$status=$status=="5"?"3":"2";
	$result=$dbproj->Execute("update m_carpool set status='$status', end_date='$date_pulang' where ocs_id='$ocs_id'");
	
	if(is_numeric($km_end1) == FALSE){
		echo json_encode(array('errorMsg'=>'Km END harus diisi!!!'));
		die();
	}
	
	if($km_start1 >= $km_end1){
		echo json_encode(array('errorMsg'=>"Isi km end dengan benar!!! $km_start1 <= $km_end1"));
		die();
	}
...
elseif($act=='combo_driver1'){
	$sql="select nip,nm_peg from m_driver where area_id in (select area_id from lmt_payment.dbo.m_kasir where pic_id='$login_nip')";
	$resultx=$db->Execute($sql);
	while($row=$resultx->Fetchrow()){
		$items[]=$row;
	}
	...
	$sql="select no_polisi,kode_kendaraan from inv_data_kendaraan where status not in ('KEMBALI') and lke_id in 
	(select area_id from lmt_payment.dbo.m_kasir where pic_id='$login_nip')";
	$resultx=$db->Execute($sql);
	while($row=$resultx->Fetchrow()){
		$items[]=$row;
	}
	echo json_encode($items);
}elseif($act=='combo_car123'){
	$q = isset($_POST['q']) ? strval($_POST['q']) : '';
	$sql="select no_polisi,kode_kendaraan from inv_data_kendaraan where lke_id in 
	(select sub_kasir from lmt_payment.dbo.m_kasir where pic_id='$login_nip' or pic_sub='$login_nip') and status='AKTIF' and (no_polisi like '%$q%')";
	$resultx=$db->Execute($sql);
	while($row=$resultx->Fetchrow()){
		$items[]=$row;
	}
