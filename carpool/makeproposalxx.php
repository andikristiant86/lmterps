...
function tambah(){
	...
	$('#dlg').dialog('open').dialog('setTitle','FORM ADD');
		$('#fm').form('clear');
		$.post('makeproposalGet.php',{act:'generate_kode'},function(result){
			var txt=result.kode;
			list=txt.split("|");
			$('#fm').form('load',{
				ocs_id:list[0]
	...
	url = 'makeproposalGet.php?act=do_add';
}
function edit(){
	$('#btnSave').linkbutton('enable');
	var row = $('#dg').datagrid('getSelected');
		if (row){
			if(row.status=='PULANG' || (row.pm_approve=='Not Approved' && row.finance_approve=='Not Approved')){
				$('#site_name').combogrid({url:'makeproposalGet.php?act=combo_site&proj_id='+row.proj_id+'&sow_id='+row.sow_id});
				...
				url = 'makeproposalGet.php?act=do_update&ocs_id='+row.ocs_id+'&status='+row.status;
				
			}else{
				$.messager.alert('Error',"Maaf untuk merubah data setelah status PULANG!",'error');
			}
	}
}function cancel_req(){
	$('#btnSave').linkbutton('enable');
		...
		if (row){
			if(row.status=='JALAN' || row.status=='PULANG' || row.status=='CLOSED'){
				...
				$.messager.prompt('Confirm','Are you sure <b>cancel?</b><br>Reason:',function(r){
					if (r){
						$.post('makeproposalGet.php?act=do_cancel&reason='+r,{status:row.status,ocs_id:row.ocs_id},function(result){
							if (result.success){
								$('#dg').datagrid('reload');
							} else {
								$.messager.alert('Error',"Request bisa di cancel, jika status BELUM JALAN!",'error');
...
if (row){
	if(row.pm_approve=='Approved' && row.finance_approve=='Approved'){
		...
		if (r){
			$.post('makeproposalGet.php?act=do_destroy',{id:row.ocs_id},function(result){
				if (result.success){
					$('#dg').datagrid('reload'); // reload the user data
				} else {
...
<table id="dg" title="MAKE PROPOSAL" class="easyui-datagrid" style="height:475px;width:100%"
data-options="
					rownumbers:true,
					singleSelect:true,
					collapsible: true,
					url:'makeproposalGet.php?act=view',
					method:'post',
					pagination:true,
					toolbar:'#toolbar',
					striped: true,
...
<div id="dlg" class="easyui-dialog" style="width:75%;height:475px;padding:0px 0px"
...
<input id="lke_id" name="lke_id" class="easyui-combobox" style="width:50%;" required="true" data-options="
        ...
        url: 'makeproposalGet.php?act=combo_lke',
		panelHeight:'auto'">
</div>
-->
<?php
...
<select class="easyui-combogrid" name="name_dt" id="name_dt" style="width:50%;"
					data-options="
							panelWidth:400,
							url: 'makeproposalGet.php?act=combo_dt&x=2',
							idField:'nip',
							textField:'nm_peg',
							mode:'remote',
							fitColumns:true,
...
<select class="easyui-combogrid" name="proj_id" style="width:50%;" required="true"
					data-options="
							panelWidth:400,
							url: 'makeproposalGet.php?act=combo_project',
							idField:'id',
							textField:'proj_name',
							mode:'remote',
							fitColumns:true,
					...
					onSelect: function (index,row){
						...
						$('#sow').combogrid({url:'makeproposalGet.php?act=combo_sow&proj_id='+row.id});
...
<label>SOW Name</label>
	...
	<!--var url = 'makeproposalGet.php?act=combo_sow1&proj_id='+row.id;
...
<div class="fitem">
<label>DT/Surveyor/Rigger</label>
<select class="easyui-combogrid" name="name_dt" id="name_dt" style="width:50%;"
					data-options="
							panelWidth:400,
							url: 'makeproposalGet.php?act=combo_dt',
							idField:'nip',
							textField:'nm_peg',
							mode:'remote',
							fitColumns:true,
...
<select class="easyui-combogrid" name="name_rno" style="width:50%;"
					data-options="
							panelWidth:400,
							url: 'makeproposalGet.php?act=combo_dt&x=1',
							idField:'nip',
							textField:'nm_peg',
							mode:'remote',
							fitColumns:true,
...
<label></label>
	 <div class="easyui-tabs" style="width:100%;height:90px" border=0>
		<div title="DT/Surveyor/Installer" style="padding:10px" border=0>
		Result: 
