...
<table id="dg" style="width:100%;height:520"
			...
			data-options="
					...
					singleSelect:true,
					collapsible: true,
					url:'RequestCarpoolGetnew.php?act=view',
					method:'post',
					pagination:true,
					toolbar:'#toolbar',
					striped: true,
	...
	<div id="toolbar">
		<table width="100%" cellpadding=0 cellspacing=0>
			...
			<td align="right">
				<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-ok" plain="true" onclick="prosess_jalan()"">Jalan</a>
...
<div id="dlg1" class="easyui-dialog" style="width:70%;height:230px;padding:5px 10px" closed="true" buttons="#dlg-buttons1">
	<form id="ff" method="post" enctype="multipart/form-data">	
		<div class="fitem">
			...
			valueField: 'nip',
			textField: 'nm_peg',
			url: '/carpool/RequestCarpoolGetnew.php?act=combo_driver1'">
        </div>
        <div class="fitem">
			...
			textField: 'no_polisi',
			url: '/carpool/RequestCarpoolGetnew.php?act=combo_car1'">
        </div>
		<div class="fitem">
	...
	<div id="hideShow">
		...
		<div class ="fitem">
		<label>Date In</label>
			<?=$f->inputDate("date_pulang","","","",true);?> Time: 
...
function prosess_jalan(){
	...
	if (row){
		if(row.status==4){
			...
			url = 'RequestCarpoolGetnew.php?act=prosess_jalan&ocs_id='+row.ocs_id;
		}else{
	...
	if (row){
		if(row.status==1||row.status==5){
			...
			$('#dlg1').dialog('open').dialog('setTitle','Carpool Information');
			url = 'RequestCarpoolGetnew.php?act=prosess_pulang&ocs_id='+row.ocs_id;	
		}else{
			$.messager.alert('Error',"Sorry, you don't have permission!",'error');
		}
	}
...
function tambah_durasi(){
	...
	$.messager.prompt('Confirm','Ingin menambah durasi? <br><b>silahkan input dengan angka:</b>',function(r){
			if (r){
				$.post('RequestCarpoolGetnew.php?act=do_tambahdurasi&durasi='+r,{ocs_id:row.ocs_id},function(result){
					if (result.success){
						$('#dg').datagrid('reload');
					} else {
