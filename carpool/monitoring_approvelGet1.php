...
if($act=='view'){
	...
	$result = array();
	$sql="select a.*,b.ocs_desc,b.date,b.km_acuan,b.uang_pulsa,d.nm_peg as pm_name, b.time,b.pm_approve_date,
	...
	$result["total"] = $dbproj->getOne("select count(*) from ($sql) as x
		where date between '$f_start_date' and '$f_end_date' and nip_app='$login_nip' $find");
		
	$_sql="select * from ($sql) as x
		where date between '$f_start_date' and '$f_end_date' and nip_app='$login_nip' $find order by status_app desc, ocs_id desc
	";
	...
	foreach ($rows as $key=>$val){
		...
		if($app_id=='1'){
			...
			$nip_app_next=$dbproj->getOne("select app_nip from m_approvel where app_no_urut='2' and app_jenis='REQ_CARPOOL' and cmp_id='".$detailuser['cmp_id']."'");
			$app_id_next=$dbproj->getOne("select app_id from m_approvel where app_no_urut='2' and app_jenis='REQ_CARPOOL' and cmp_id='".$detailuser['cmp_id']."'");
